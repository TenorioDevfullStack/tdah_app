    ...
    if os.path.exists(TASKS_FILE):
        ...
        if not df.empty:
            ...
            if "completed" in df.columns:
                # converte "True"/"False"/1/0/yes/no para bool
                df["completed"] = (
                    df["completed"]
                    .astype(str)
                    .str.strip()
...
# -----------------------------------------------------------------------------
# Util: carregar e salvar Finanças
# -----------------------------------------------------------------------------
def load_finance():
...
<<<<<<< HEAD
    ...
    if not tasks_df.empty:
        def highlight_row(row):
            try:
                overdue = (not row["completed"]) and (pd.notna(row["due_date"])) and (row["due_date"] < pd.Timestamp.now())
            except Exception:
                overdue = False
...
<<<<<<< HEAD
    ...
    if not tasks_df.empty:
        incomplete = tasks_df[tasks_df["completed"] == False]
        if not incomplete.empty:
            choice = st.selectbox("Selecione a tarefa a concluir", incomplete["description"].tolist())
            if st.button("Marcar como concluída"):
                idx = tasks_df[tasks_df["description"] == choice].index[0]
...
def page_settings():
    ...
    st.markdown("Algumas configurações dependem do tema do Streamlit.")

    theme = st.selectbox("Tema", ["Claro", "Escuro"])
    font_size = st.slider("Tamanho da fonte (px)", 14, 22, 16)
